...
$columns = array(
    ...
    array(
        ...
        'formatter' => function($d, $row ) {
            global $functions;
            return $functions->textModal($row["not_text"],40);
        }
    ),
    array(
        'db'        => 'created_at',
    ...
    array(
        ...
        'formatter' => static function($d, $row ) {
            ...
            if ($session->sessionRoleControl($pageRoleKey, $constants::deletePermissionKey) === true){
                $exportButton .= '
                    <button type="button" class="btn btn-outline-danger m-1 post_delete" data-delete-url="'.$system->adminUrl("ajax/delete-email-theme?delete=" . $row["id"]).'">
