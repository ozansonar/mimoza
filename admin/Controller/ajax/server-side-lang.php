...
$columns = array(
    ...
    array(
        ...
        'formatter' => function($d, $row ) {
            global $functions;
            return $functions->textModal($row["lang"], 20);
        }
    ),
    array( 'db' => 'short_lang', 'dt' => 'short_lang' ),
    array(
        'db'        => 'default_lang',
        'dt'        => 'default_lang',
        'formatter' => function($d, $row ) {
            global $constants;
            return array_key_exists($row["default_lang"], $constants::systemYesAndNoText)
    ...
    array(
        ...
        'formatter' => function($d, $row ) {
            global $constants;
            return array_key_exists($row["form_validate"], $constants::systemYesAndNoText)
    ...
    array(
        ...
        'formatter' => static function($d, $row ) {
            ...
            if ($session->sessionRoleControl($pageRoleKey, $constants::deletePermissionKey) === true){
                $exportButton .= '
                    <button type="button" class="btn btn-outline-danger m-1 post_delete" data-delete-url="'.$system->adminUrl("ajax/delete-lang?delete=" . $row["id"]).'">
