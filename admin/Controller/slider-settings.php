...
if (isset($_POST["submit"]) && $_POST["submit"] == 1) {
	...
	foreach ($projectLanguages as $project_languages_row) {
		...
		$page_data[$project_languages_row->short_lang]["link"] = $functions->clean_post("link");
		...
		$page_data[$project_languages_row->short_lang]["site_disi_link"] = $functions->post("site_disi_link") ? 1 : 0;
		$page_data[$project_languages_row->short_lang]["back_link"] = $functions->clean_post("back_link");
		$page_data[$project_languages_row->short_lang]["yeni_sekme"] = $functions->post("yeni_sekme") ? 1 : 0;
		...
		if ($project_languages_row->form_validate == 1) {
			...
			if(empty($page_data[$project_languages_row->short_lang]["text"])){
				...
				if(strlen(strip_tags($page_data[$project_languages_row->short_lang]["text"])) < 10){
					$message["reply"][] = $project_languages_row->lang." - İçerik 10 karekterden az olamaz.";
				}
			}
			*/

			if ($page_data[$project_languages_row->short_lang]["site_disi_link"] == 1 && empty($page_data[$project_languages_row->short_lang]["site_disi_link"])) {
				$message["reply"][] = $project_languages_row->lang . " - Lütfen site dışı linki yazınız.";
			}

			if (!empty($page_data[$project_languages_row->short_lang]["site_disi_link"])) {
				if (!filter_var($page_data[$project_languages_row->short_lang]["back_link"], FILTER_VALIDATE_URL)) {
	...
	if (empty($message)) {
		...
		foreach ($projectLanguages as $project_languages_row) {
			...
			$db_data["link"] = !empty($page_data[$project_languages_row->short_lang]["link"]) ? $functions->permalink($page_data[$project_languages_row->short_lang]["link"]) : $functions->permalink($page_data[$project_languages_row->short_lang]["title"]);
			...
			$db_data["site_disi_link"] = $page_data[$project_languages_row->short_lang]["site_disi_link"];
			$db_data["back_link"] = $page_data[$project_languages_row->short_lang]["back_link"];
			$db_data["yeni_sekme"] = $page_data[$project_languages_row->short_lang]["yeni_sekme"];
			...
			if ($id > 0) {
				if (array_key_exists($project_languages_row->short_lang, $db_data_lang)) {
					...
					//güncelleme
					$update = $db::update("slider", $db_data, array("id" => $page_data[$project_languages_row->short_lang]["id"]));
				} else {
