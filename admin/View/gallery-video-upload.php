                ...
                <table  id="datatable-1" class="table table-bordered table-striped">
                    ...
                    <?php foreach ($videos_data as $row): ?>
                        <tr>
                            ...
                            <td>
                                <?php if($session->sessionRoleControl($page_role_key,$addPermissionKey) == true): ?>
                                    <button type="button" class="btn btn-outline-success m-1" onclick="post_edit('<?php echo $adminSystem->adminUrl("gallery-video-upload?id=".$row->gallery_id."&video_id=".$row->id); ?>')"><i class="fas fa-pencil-alt px-1 px-1"></i></i>Düzenle</button>
                                <?php endif; ?>
                                <?php if($session->sessionRoleControl($page_role_key,$addPermissionKey) == true): ?>
                                    <button type="button" class="btn btn-outline-danger  m-1" onclick="post_delete('<?php echo $adminSystem->adminUrl("gallery-video-upload?id=".$row->gallery_id."&delete=".$row->id); ?>')"><i class="fas fa-trash px-1 px-1"></i> Sil</button>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
