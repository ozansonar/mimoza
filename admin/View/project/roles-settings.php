        ...
        <section class="content">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form  enctype="multipart/form-data" method="post" id="pageForm">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control validate[required]" name="group_name" id="id_group_name" placeholder="<?php echo $admin_text["ADMIN_ROLE_ROLE_NAME"] ?>" value="<?php echo isset($group_name) ? $group_name:(isset($data) ? $data->group_name:null); ?>">
                        </div>
                        ...
                        <div class="row">
                                    ...
                                    <div class="form-group mb-3 mb-md-2">
                                        <?php if(isset($menu["submenu"])): ?>
                                            <div class="icheck-primary d-inline">
                                                <input type="checkbox" class="<?php echo $rand ?>_top" id="id_permission_<?php echo $menu["url"]."_s"; ?>" name="permissions[<?php echo $menu["url"]; ?>][<?php echo "s"; ?>]" value="<?php echo "s"; ?>" <?php echo isset($role_array[$menu["url"]]) && in_array("s",$role_array[$menu["url"]]) ? "checked":null; ?>>
                                                            ...
                                                            <div class="row pl-2 mt-1">
                                                                <?php foreach ($submenu["permissions"] as $per_key=>$per_val): ?>
                                                                    <div class="icheck-primary d-inline pr-4 <?php echo $per_key != 0 ? "pl-4":null;  ?>">
                                                                        <input type="checkbox" class="check-click" data-permission-category="<?php echo $rand; ?>" id="id_permission_<?php echo $submenu["url"]."_".$per_key; ?>" name="permissions[<?php echo $submenu["url"]; ?>][<?php echo $per_key; ?>]" value="<?php echo $per_key; ?>" <?php echo isset($role_array[$submenu["url"]]) && in_array($per_key,$role_array[$submenu["url"]]) ? "checked":null; ?>>
                                                    ...
                                                    <div class="row pl-2 mt-1">
                                                        <?php foreach ($menu["permissions"] as $per_key=>$per_val): ?>
                                                            <div class="icheck-primary d-inline pr-4">
                                                                <input type="checkbox" id="id_permission_<?php echo $menu["url"]."_".$per_key; ?>" name="permissions[<?php echo $menu["url"]; ?>][<?php echo $per_key; ?>]" value="<?php echo $per_key; ?>" <?php echo isset($role_array[$menu["url"]]) && in_array($per_key,$role_array[$menu["url"]]) ? "checked":null; ?> >
                            ...
                            <?php endforeach; ?>
                            <?php foreach ($extra as $extra_row): ?>
                                <div class="col-12 table-bordered m-2 p-2">
                                    ...
                                    <div class="row p-0 m-0">
                                        <?php foreach ($extra_row["permissions"] as $permission_extra_key=>$permission_extra): ?>
                                            <div class="icheck-primary d-inline pr-4">
                                                <input type="checkbox" id="id_permission_<?php echo $extra_row["url"]."_".$permission_extra_key; ?>" name="permissions[<?php echo $extra_row["url"]; ?>][<?php echo $permission_extra_key; ?>]" value="<?php echo $permission_extra_key; ?>" <?php echo isset($role_array[$extra_row["url"]]) && in_array($permission_extra_key,$role_array[$extra_row["url"]]) ? "checked":null; ?>>
                            ...
                            if(!empty($onyuz_yetkiler)):
                            ?>
                                <div class="col-12 my-2"><h3>Önyüz Yetkileri</h3></div>
                                <?php foreach ($onyuz_yetkiler as $on_key => $on_row): ?>
                                <?php $rand = rand(1,99999); ?>
                                <div class="col-12 table-bordered m-2 p-2">
                                    <div class="form-group mb-3 mb-md-2">
                                        <?php if(isset($on_row["submenu"])): ?>
                                            <div class="icheck-primary d-inline">
                                                <input type="checkbox" class="<?php echo $rand ?>_top" id="id_permission_<?php echo $on_row["url"]."_s"; ?>" name="permissions[<?php echo $on_row["url"]; ?>][<?php echo "s"; ?>]" value="<?php echo "s"; ?>" <?php echo isset($role_array[$on_row["url"]]) && in_array("s",$role_array[$on_row["url"]]) ? "checked":null; ?>>
                                                ...
                                                <?php if(isset($on_row["permissions"])): ?>
                                                    <div class="row pl-2 mt-1">
                                                        <?php foreach ($on_row["permissions"] as $per_key=>$per_val): ?>
                                                            <div class="icheck-primary d-inline pr-4">
                                                                <input type="checkbox" id="id_permission_<?php echo $on_row["url"]."_".$per_key; ?>" name="permissions[<?php echo $on_row["url"]; ?>][<?php echo $per_key; ?>]" value="<?php echo $per_key; ?>" <?php echo isset($role_array[$on_row["url"]]) && in_array($per_key,$role_array[$on_row["url"]]) ? "checked":null; ?> >
                ...
                <div class="card-footer text-right">
                    <button type="submit" name="submit" form="pageForm" value="1" class="btn btn-success ml-3">Kaydet <i class="fas fa-save ml-2"></i></button>
                </div>
                <!-- /.card-footer-->
            </div>
            <!-- /.card -->
