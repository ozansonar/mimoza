...
class FileUploader{
	 ...
	 * @var string[]
	 */
	public $compressor_extension = array("png","jpg","jpeg");
    //sıkıştırma leveli
	/**
	 * @var int
	 */
	public $compressor_level = 4;
	 ...
	 */
	function __construct($file_type_path){
        ...
        if(isset($_FILES[$this->global_file_name])){
            ...
            if ($handle->uploaded) {
                ...
                if ($handle->processed){
                    ...
                    if($this->compressor && in_array($extension,$this->compressor_extension)){
                        if(!file_exists($url."compressed")){
                            mkdir($url."compressed",0777);
                        }
                        // setting
                        $setting_img = array(
